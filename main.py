...
import argparse, inspect, os, pickle, random, shutil, sys, time
...
def get_parser():
    ...
    parser = argparse.ArgumentParser(description='ST-GAT')
    ...
    parser.add_argument('--save-score', type=str2bool, default=False, help='if true, the classification score will be stored')
    ...
    # optim
    parser.add_argument('--base-lr', type=float, default=0.05, help='initial learning rate')
    ...
    parser.add_argument('--weight-decay', type=float, default=0.001, help='weight decay for optimizer')
...
class Processor():
    ...
    def load_data(self):
        ...
        if self.arg.phase == 'train':
            self.data_loader['train'] = torch.utils.data.DataLoader(dataset=Feeder(**self.arg.train_feeder_args),
                ...
                drop_last=True, worker_init_fn=init_seed, pin_memory=True)
        self.data_loader['test'] = torch.utils.data.DataLoader(dataset=Feeder(**self.arg.test_feeder_args),
            batch_size=self.arg.test_batch_size, shuffle=False, num_workers=self.arg.num_worker,
            drop_last=False, worker_init_fn=init_seed, pin_memory=True)

    def load_model(self):
        output_device = self.arg.device[0] if type(self.arg.device) is list else self.arg.device
        self.output_device = output_device
        Model = import_class(self.arg.model)                                    # Load the object Model() in ctrgcn
        shutil.copy2(inspect.getfile(Model), self.arg.work_dir)
        print(Model)
        self.model = Model(**self.arg.model_args)                               # Set the args for the object
        # print(self.model)
        self.loss = nn.CrossEntropyLoss().cuda(output_device)                   # Init loss function
    ...
    def load_optimizer(self):
        if self.arg.optimizer == 'SGD':
            self.optimizer = optim.SGD(self.model.parameters(), lr=self.arg.base_lr,
                momentum=0.9, nesterov=self.arg.nesterov, weight_decay=self.arg.weight_decay)
        elif self.arg.optimizer == 'Adam':
            self.optimizer = optim.Adam(self.model.parameters(), lr=self.arg.base_lr,
    ...
    def train(self, epoch, save_model=True):
        self.model.train()                                              # set the "train" flag
    ...
    def eval(self, epoch, save_score=False, loader_name=['test'], wrong_file=None, result_file=None):
        ...
        self.model.eval()                                               # set the "eval" flag
    ...
    def start(self):
        if self.arg.phase == 'train':
            ...
            self.eval(epoch=-1, save_score=True, loader_name=['test'], wrong_file=wf, result_file=rf)
            ...
            self.eval(epoch=-1, save_score=self.arg.save_score, loader_name=['test'], wrong_file=wf, result_file=rf)
            self.print_log('Done.\n')
...
if __name__ == '__main__':
    ...
    p = parser.parse_args()
    if p.config is not None:                        # if config file is provided in the call
        with open(p.config, 'rb') as f:
            default_arg = yaml.safe_load(f)         # load arguments from config file
        key = vars(p).keys()
        for k in default_arg.keys():
            if k not in key:                        # incorrect args are not allowed
                print('WRONG ARG: {}'.format(k))
                assert (k in key)
        parser.set_defaults(**default_arg)          # set all default args to the loaded ones

    arg = parser.parse_args()                       # parse newly loaded default args
    init_seed(arg.seed)                             # seed the RNG to [1]
    processor = Processor(arg)                      # call up Processor class with the loaded args
    processor.start()                               # call class function start()
