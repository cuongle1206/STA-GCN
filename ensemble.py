...
if __name__ == "__main__":
    ...
    parser.add_argument('--joint-dir',
                        help='Directory containing "epoch0_test_score.pkl" for joint eval results')
    parser.add_argument('--bone-dir',
                        help='Directory containing "epoch0_test_score.pkl" for bone eval results')
    ...
    with open(os.path.join(arg.joint_dir, 'epoch0_test_score.pkl'), 'rb') as r1:
        r1 = list(pickle.load(r1).items())

    with open(os.path.join(arg.bone_dir, 'epoch0_test_score.pkl'), 'rb') as r2:
        r2 = list(pickle.load(r2).items())

    if arg.joint_motion_dir is not None:
        with open(os.path.join(arg.joint_motion_dir, 'epoch0_test_score.pkl'), 'rb') as r3:
            r3 = list(pickle.load(r3).items())
    if arg.bone_motion_dir is not None:
        with open(os.path.join(arg.bone_motion_dir, 'epoch0_test_score.pkl'), 'rb') as r4:
